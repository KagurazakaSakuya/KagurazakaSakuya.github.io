...
$.ajax({
  ...
  data: {
      key: 'GWABZ-753E4-7PWUF-X4E5U-QE633-RKBZE',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
    ...
    let dist = getDistance(121.636417, 38.88748, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("02/27/2020 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 大聪明！ %c 你正在访问Sakuyaの小屋",
...
if (m == 12 && dd == 16) {//站长生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("Sakuya今天" + (y - 2004).toString() + "岁啦~！");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
if (m == 6 && dd == 30) {//小猫咪生日
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("02/27/2020 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<br> <div style="font-size:13px;font-weight:bold">本站已经在这个世界上存在了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
...
function setFont(n) {
  ...
  if (n == "default") {
      document.documentElement.style.setProperty('--global-font', 'ZhuZiAWan');
      document.body.style.fontFamily = "ZhuZiAWan, Consolas_1, BlinkMacSystemFont, 'Segoe UI' , 'Helvetica Neue' , Lato, Roboto, 'PingFang SC' , 'Microsoft JhengHei' , 'Microsoft YaHei' , sans-serif";
  }
  else {
    document.documentElement.style.setProperty('--global-font', n);
      document.body.style.fontFamily = "var(--global-font),ZhuZiAWan, IBM Plex Mono ,monosapce,'微软雅黑', sans-serif";
  }
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://bu.dusays.com/2023/07/20/64b9341a3f00a.jpg);
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://i.328888.xyz/2023/03/11/smRpv.jpeg"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
